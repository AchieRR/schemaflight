...
class StubEntities:
    def __init__(self) -> None:
        self.upserted: list[tuple[StubDataset, Any]] = []

    def upsert(self, dataset: StubDataset, *, emit_mode: Any) -> None:
        self.upserted.append((dataset, emit_mode))
...
def test_demo_seed_builds_tagged_two_hop_column_lineage() -> None:
    ...
    assert [item.payload["name"] for item, _ in client.entities.upserted] == [
        "shop.customer",
        "analytics.customer_360",
        "growth.retention_export",
    ]
    source, model, consumer = [item for item, _ in client.entities.upserted]
    assert [mode for _, mode in client.entities.upserted] == ["sync-wait"] * 3
